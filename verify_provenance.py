#!/usr/bin/env python3
"""
verify_provenance.py - Unified command line tool for verifying document and analysis provenance
"""
...
def verify_document(doc_id: str, repo_dir: str, passphrase: str) -> int:
    ...
    repo = DocumentRepository(base_dir=repo_dir, dev_mode=True)
...
def verify_analysis(analysis_id: str, repo_dir: str, passphrase: str) -> int:
    """Verify an analysis record's provenance"""
    repo = DocumentRepository(base_dir=repo_dir, dev_mode=True)
    
    result = repo.verify_analysis(analysis_id)
...
def verify_all(repo_dir: str, passphrase: str) -> int:
    """Verify all documents and analyses in the repository"""
    repo = DocumentRepository(base_dir=repo_dir, dev_mode=True)
    
    results = repo.verify_all()
    
    print(f"\n=== Repository Verification Summary ===")
    print(f"\nDocuments:")
    print(f"  Total: {results['documents']['total']}")
    print(f"  Verified: {results['documents']['verified']}")
    print(f"  Failed: {results['documents']['failed']}")
    
    print(f"\nAnalyses:")
    print(f"  Total: {results['analyses']['total']}")
    print(f"  Verified: {results['analyses']['verified']}")
    print(f"  Failed: {results['analyses']['failed']}")
    
    if results['failures']:
        print(f"\nFailure details:")
        for failure in results['failures']:
            print(f"  - {failure['type']} {failure['id']}: {failure['reason']}")
        return 1
...
def main() -> int:
    ...
    analysis_parser.add_argument("--repo-dir", default="data/documents", help="Document repository directory")
    
    # Verify all
    all_parser = subparsers.add_parser("verify-all", help="Verify all documents and analyses")
    ...
    if args.command == "verify-document":
        ...
        return verify_analysis(args.analysis_id, args.repo_dir, passphrase)
